...
% If cov == true
%    then M is the covariance and mu_r is the mean 
% if cov== false 
%    then M is the precision matrix and mu_r is the co-vector M * mu 
%       (the log-density is -1/2 X'*M*X + mu_r'*X)
%
% If the function is used as a Gibbs step, set L = 2 such that the 1st column of Xs
% is the previous draw and the 2nd column of Xs is the new draw.
...
% Output
% Xs:      d x L matrix, each column is a sample (the 1st column is initial_X)
...
m = size(g,1);
d = size(initial_X,1);
if or(size(F,1) ~= m, size(F,2)~=d)
    error('wrong size of F');
end    
% Xs=NaN;
bounce_count =0;
nearzero= 10000*eps;

if cov %mean and covariance matrix
    mu= mu_r;
    g = g + F*mu;
    R = chol(M);
    F = F*R';
...
else %co-vector and precision matrix
    r=mu_r;
    R=chol(M);      % M = R'*R    
    mu = R\(R'\r);
    g = g+F*mu;
...
% Verify that initial_X is feasible
 c= F*initial_X +g;
 if any(c<0)
     disp('error: inconsistent initial condition');
     return
 end
...
%        fprintf(num2str(tt/T));
    ...
    if all(F*X +g > 0)
        ...
        disp('hmc reject: check numerical issue')
