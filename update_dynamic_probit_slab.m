...
function [z, alpha, zi, ind_alpha] = update_dynamic_probit_slab(y, x, z, alpha, zi, ...
    zi_prior_mean, zi_prior_cov_inv, ind_alpha, c, s_alpha, q_alpha, var_const)
% Inputs:
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Update indictor of spike-slabe prior for beta
vary = ones(n,1);
ind_alpha = SpikeSlab_PriorUpdate_ind_marg_single(z-x(:,1)*beta(1), ...
    [x(:,2:K)  [zi;z(1:n-1)]], vary, ind_alpha, c, s_alpha, q_alpha);
psil = [var_const; ind_alpha.*s_alpha + c*(1-ind_alpha).*s_alpha];
alpha_prior_cov_inv = diag(1./psil);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
