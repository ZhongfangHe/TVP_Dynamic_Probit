...
%
% marginal draw for tau, taul; adaptive MH
% tau, taul are based on double cauchy


function draws = Est_DProbTVP_HS_dc(y, x, burnin, ndraws, ind_sparse, ind_pred)
% Inputs:
...
pstar = 0.44; %target MH acceptance rate
logtau_mean = 0;
logtau_var = 0;
logrw_tau = 0;
logrw_start_tau = logrw_tau;
drawi_start_tau = 0; %tau
logtaul_mean = 0;
logtaul_var = 0;
logrw_taul = 0;
logrw_start_taul = logrw_taul;
drawi_start_taul = 0; %taul


%% MCMC
draws.count_tau = 0;
draws.logrw_tau = zeros(ndraws,1); %tau for v
draws.count_taul = 0;
draws.logrw_taul = zeros(ndraws,1); %taul for beta0
...
draws.beta0_d = zeros(ndraws,2*K+2);

draws.v = zeros(ndraws,K+1);
draws.v_d = zeros(ndraws,2*K+4);
...
%     beta0v = mvnrnd(beta0v_b, beta0v_B)';    
    if rcond(beta0v_B_inv) > 1e-15 
        beta0v_B = beta0v_B_inv\eye(2*K+2);
        beta0v_b = beta0v_B * (xx' * z);
        beta0v = mvnrnd(beta0v_b, beta0v_B)';
    else
        beta0v_B = robust_inv(beta0v_B_inv);
        beta0v_B_half = robust_chol(beta0v_B);
        beta0v_b = beta0v_B * (xx' * z);
        beta0v = beta0v_b + beta0v_B_half * randn(2*K+2,1);
    end   
    beta0 = beta0v(1:K+1);
    v = beta0v(K+2:2*K+2);
...
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++    
    ...
    a0v_prior_var = [psil(1:K); psi(1:K); z0_prior_var];
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    v2 = v.^2;
    [tau, count_tau, logrw_tau, drawi_start_tau, logrw_start_tau] = ...
        AMH_HS_dc(v2, tau,...
...
%         tauj, pstar, logrw_tau, drawi_start_tau, logrw_start_tau, ...
%         drawi, burnin, 1/1);    
    [tauj, tauj_d] = Horseshoe_update_local_vector(v2, tau, tauj, tauj_d);
...
% %         tauj, tauj_d, 1/n);
    ...
    [taul, count_taul, logrw_taul, drawi_start_taul, logrw_start_taul] = ...
        AMH_HS_dc(beta02, taul,...
        phil, pstar, logrw_taul, drawi_start_taul, logrw_start_taul, ...
        drawi, burnin);     
%     [taul, count_taul, logrw_taul, drawi_start_taul, logrw_start_taul] = ...
%         AMH_HS_tau(beta02, taul,...
%         phil, pstar, logrw_taul, drawi_start_taul, logrw_start_taul, ...
%         drawi, burnin, 1/1);    
    [phil, phil_d] = Horseshoe_update_local_vector(beta02, taul, phil, phil_d);
...
% %         phil, phil_d, 1/n); 
    ...
    if drawi > burnin
        ...
        draws.count_tau = draws.count_tau + count_tau/ndraws;
        draws.logrw_tau(i) = logrw_tau; %tau for v
        draws.count_taul = draws.count_taul + count_taul/ndraws;
        draws.logrw_taul(i) = logrw_taul; %taul for beta0        
