...
% ind for the first element for x is not needed

function [ind_x, ind_z] = SpikeSlab_PriorUpdate_ind_probit2(y, x, z, sig2, c, ind_x, ind_z, ...
    s_x, s_z, q_x, q_z, lambda)
% Inputs:
...
ind = [ind_x;  ind_z];
s = [s_x; s_z];
for j = 1:(K-1) %iterate over possible scenarios
    indj = ind;
    
    indj(j) = 1;
    psi = [lambda; indj.*s + c * (1-indj).*s]; %first element has a fixed variance lambda
